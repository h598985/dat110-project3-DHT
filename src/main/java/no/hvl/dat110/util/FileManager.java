...
public class FileManager {
	...
	public void createReplicaFiles(){
		...
		for (int i = 0; i < numReplicas; i++) {
			...
			//File replica = new File(filepath + "/" + replicaFilename + ".txt");


			/*try {
				replica.createNewFile();

				OutputStream out = new FileOutputStream(replica);

				out.write(bytesOfFile);

			} catch (IOException e) {
				logger.error("Failed to write replica file: {}", replicaFilename);
				e.printStackTrace();
			}
*/
			// hash the replica
    ...
    public int distributeReplicastoPeers() throws RemoteException {
		...
		// implement a logic to decide if this successor should be assigned as the primary for the file
			...
			if(i == index){
				successor.saveFileContent(filename,replicaHash, bytesOfFile, true);
			} else{
				successor.saveFileContent(filename,replicaHash, bytesOfFile, false);
			}
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (int i = 0; i < numReplicas; i++) {
			...
			BigInteger replicaHash = replicafiles[i];
			NodeInterface successorNode = chordnode.findSuccessor(replicaHash);

			// get the metadata (Message) of the replica from the successor (i.e., active peer) of the file
			Message metadata = successorNode.getFilesMetadata(replicaHash);
	...
	public NodeInterface findPrimaryOfItem() {
		...
		try {
			Set<Message> replicas = requestActiveNodesForFile(filename);

			// iterate over the activeNodesforFile

			for (Message replica : replicas) {
				...
				if (replica.isPrimaryServer()) {
					...
					primaryNode = Util.getProcessStub(replica.getNodeName(), replica.getPort());
				}
